...
export const WhaleComponent = ({
  ...
  const whaleSize = Math.min(
    Math.max(
      minSize +
        ((transaction.usdValue - minUsdAmount) /
          (maxUsdAmount - minUsdAmount)) *
          (maxSize - minSize),
      minSize
    ),
    maxSize
      ...
      <TooltipProvider>
        <Tooltip delayDuration={0}>
          ...
          <TooltipContent side='top' align='start' sideOffset={5}>
            ...
            <p>
              From: {transaction.sender.slice(0, 6)}...
              {transaction.sender.slice(-4)}
            </p>
            <p>
              To: {transaction.receiver.slice(0, 6)}...
