...
const WhaleComponent = ({ transaction, onComplete }: { transaction: Transaction; onComplete: () => void }) => {
  ...
  // Calculate whale size based on transaction amount with increased sizes
  const minSize = 80 // Minimum whale size
  const maxSize = 400 // Maximum whale size
  const minAmount = 100000 // Minimum transaction amount (for smallest whale)
  const maxAmount = 10000000 // Maximum transaction amount (for largest whale)
...
export default function Component() {
  ...
  const generateNewYPosition = useCallback(() => {
    const minDistance = 20 // Minimum distance between whales
    let newY = lastYPosition.current + minDistance
    if (newY > 80) { // Reduced maximum Y position to accommodate larger whales
      newY = 10 // Reset to top if we've reached the bottom
    }
    lastYPosition.current = newY
    return newY
  }, [])

  // Simulating webhook data reception
  useEffect(() => {
    const simulateWebhook = () => {
      const newTransaction: Transaction = {
        id: Date.now(),
        amount: Math.floor(Math.random() * 9900000) + 100000, // Random amount between 100,000 and 10,000,000
        timestamp: new Date(),
      ...
      setWhales(prevWhales => [...prevWhales, newTransaction])
      setTransactions(prevTransactions => [newTransaction, ...prevTransactions.slice(0, 9)]) // Keep last 10 transactions
    }

    // Simulate receiving webhook data every 5-15 seconds
          ...
          <TableBody>
            {transactions.map((transaction) => (
              <TableRow key={transaction.id}>
                ...
                <TableCell className="py-1">{transaction.timestamp.toLocaleTimeString()}</TableCell>
